...
for newsize, newpad, newcolor, imglabel in zip(NEWSIZE, NEWPAD, NEWCOLOR, IMGLABEL):
    ...
    plt.savefig(new_dir_plots+'/NeVOmics_Plot_Network_'+str(imglabel)+'.svg')
...
NEWCOLOR = ['white', colorletra]
IMGLABEL = [1, 2]
#pos = nx.spring_layout(G, k = 0.4, iterations=50, threshold=0.001, seed= 123)
pos = nx.kamada_kawai_layout(G, dist=None, weight='weight', scale=1, center=None, dim=2)
for newsize, newpad, newcolor, imglabel in zip(NEWSIZE, NEWPAD, NEWCOLOR, IMGLABEL):
    print(imglabel)
    fig = plt.figure(figsize=(15, 7))
    ax = fig.add_axes([0, 0, 0.02, 0.07])
    
    nx.draw_networkx_nodes(G, pos, nodelist = paths,
                           node_size =  list(np.array(sizenodo)*2),
                           node_shape = 'o',
                           node_color = edge_colors[usercolormap][0:len(paths)],
                           linewidths = 0, edgecolors= 'white')
    
    nx.draw_networkx_nodes(G, pos, nodelist = keggtabla.label.tolist(),
                           node_size =  0.5,
                           node_shape = 'o',
                           node_color = 'blue',
                           linewidths = 0, edgecolors= 'white')
    nx.draw_networkx_labels(G,pos, labels  =dict(zip(paths, paths)),font_size=.1, font_color='black')

    nx.draw_networkx_edges(G, pos, width = 0.3, alpha= 0.5, edge_color= verti)
    ax.axis('equal')
    ...
    #plt.savefig(new_dir_plots+'/Network2.png', dpi = 600, bbox_inches=extent, pad_inches = 0, transparent=True)
    plt.savefig(new_dir_plots+'/Network3.svg',bbox_inches=extent, pad_inches = 0, transparent=True)
    
    #plt.savefig('NeVOmics_Plot_Network_'+str(imglabel)+'.svg')
    #plt.savefig(level_2_kegg+'NeVOmics_Plot_Network_'+str(imglabel)+'.png', dpi = 900, bbox_inches='tight')
    plt.show()
# In[ ]:
...
for lll, imglabel in zip([0, 1], IMGLABEL):
    ...
    plt.savefig(new_dir_plots+'/NeVOmics_Plot_Bar_'+str(imglabel)+'.svg')
    #plt.savefig(level_2_kegg+'NeVOmics_Plot_Bar_'+str(imglabel)+'.png', dpi = 900, bbox_inches='tight')
...
for abrir, imglabel in zip([0, 0.5], IMGLABEL):
    ...
    plt.savefig(new_dir_plots+'/NeVOmics_Plot_Circle_'+str(imglabel)+'.svg')
    #plt.savefig(level_2_kegg+'NeVOmics_Plot_Circle_'+str(imglabel)+'.png', dpi = 900, bbox_inches='tight')
