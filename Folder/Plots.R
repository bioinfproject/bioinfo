...
if (ncol(nodes) == 7) {
  ...
  if (total.terms <= 60) {
    ...
    if (total.terms <= 30) {
      ...
      # |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
      #
      # |||||||||     1)    simple Plots with Terms as label
      #
      # |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
      # |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
      #
      ...
      for (i in layouts){
        ...
        graf1 = graf1 + scale_color_gradient(low = c("green","black"), high = "red") +
          labs(caption = paste("NeVOmics:",sep=" ",format(Sys.time(),"%a(%d)/%b/%Y   %X")))
        {
          png(file=paste("zxcvbnmPlot_Network_Des_Layout_",i,".png"),
              width = width.png,  #<---------------------
              height = 15, 
              units = "in",
              res=600,bg="white")
      ...
      for (i in layouts){
        ...
        graf1 = graf1 + scale_color_gradient(low = c("green","black"), high = "red") +
          labs(caption = paste("NeVOmics:",sep=" ",format(Sys.time(),"%a(%d)/%b/%Y   %X")))
        {
          png(file=paste("zxcvbnmPlot_Network_Id_Layout_",i,".png"),
              width = width.png,  #<--------------------- 
              height = 15, 
              units = "in",
              res=600,bg="white")
      ...
      for (i in layouts){
          ...
          labs(caption = paste("NeVOmics:",sep=" ",format(Sys.time(),"%a(%d)/%b/%Y   %X")))
        {
          ...
          ## Save plot
          png(file=paste("zxcvbnmPlot_Network_Bar_Des_Layout_",i,".png"),
              width = 25,
              height = 15,
              units = "in",
              res=600,bg="white")
      ...
      for (i in layouts){
          ...
          labs(caption = paste("NeVOmics:",sep=" ",format(Sys.time(),"%a(%d)/%b/%Y   %X")))
        {
          ...
          plot.with.inset = ggdraw() +
            ...
            draw_plot(barplot.enrichment, .71,.01, .25, .825)
          ## Save plot
          png(file=paste("zxcvbnmPlot_Network_Bar_Id_Layout_",i,".png"),
              width = 25,
              height = 15,
              units = "in",
              res=600,bg="white")
      ...
      if ((vcount(link_tbl) > 1) && (vcount(link_tbl) <= 45)) {
        ...
        for (i in -1:0) {
          ...
          #
          {
            ...
            text(x=-.93,y= .75, "logFC", adj = c(0,0) ,font = 2, pos = 3,
