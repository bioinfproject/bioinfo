...
if file_uniprot == []:
    ...
    acc_uniprot_GO_id=pd.read_csv(new_folder+'/annotation_'+Prefix,sep='\t').rename(columns={'GO IDs':'GO'}).dropna().reset_index(drop=True)
else:
    if os.path.exists(file_uniprot[0]):
        #print('ya existe')
        acc_uniprot_GO_id=pd.read_csv(file_uniprot[0],sep='\t').rename(columns={'GO IDs':'GO'}).dropna().reset_index(drop=True)
